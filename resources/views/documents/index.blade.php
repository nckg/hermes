...
@section('content')
    <overview inline-template
              :tags="{{ $tags }}">
        <div>
            <div class="transition-s absolute bg-smoke-light pin z-10 is-loading" v-if="isWorking"></div>
            <div class="flex">
                <div class="w-3/4">
                    <div class="shadow bg-white border border-grey-light rounded p-4">
                        ...
                        >
                            ...
                            <table-column show="title"
                                          label="{{ __("Title") }}">
                                <template slot-scope="{ id, title }">
                                    <a class="text-blue hover:text-blue-dark" :href="route('documents.show', { id })">@{{ title }}</a>
                                </template>
                            </table-column>
